...
|---|---|---|
| **A. "AI marketing platform"** | The folder that already exists in buyers' heads; zero explanation needed | Crowded folder; anchors price to the A15 tool band (Holo's position — `competitor-holo.md` pricing table; third-party-sourced, spot-check before any external use — **AMENDED 2026-09-05, shift 23: the spot-check was performed. Phin's 3 Sep primary capture confirms the $20/$48 list and $12/$29 promo band exactly, so the band this line rests on is the one claim the teardown warned about and the one the primary confirms; six other claims of its own are FALSE. `assumptions.md` A15 (SOURCED, ruled 2026-09-07), receipt `gtm/raise/model/snapshots/2026-09-07-shift25-holo-teardown-grade.md` (the pulled §4's re-cut). Do not read the teardown's flag as covering anything but its pricing**). $299 reads 6× too expensive in this folder |
...
- **Buy** (Holo and the tool band) — **REWRITTEN AT SOURCE 2026-09-03
  ...
  posts, measures and ships a Learnings surface; A15 was upgraded to OBSERVED here and **the truth table ~~now reads it UNSETTLED (amended 2026-09-05, shift 23)~~ reads it SOURCED — RULED 2026-09-07 by the chief, law 7 (see the note at §5's price-anchor row); the figures are unchanged and are Phin's primary**, and its
