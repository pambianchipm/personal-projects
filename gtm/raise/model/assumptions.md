...
| A15 | SMM tool band (Holo-class) | **SOURCED — list $20 / $48, promo $12 / $29, TWO plans only, 120 / 350 creatives per month. Document: Phin's primary capture of `tryholo.ai/pricing`, `gtm/market/competitors/snapshots/2026-09-03-holo-primary.md` §1, captured 2026-09-03.** *(Figures unchanged since shift 13. History, kept: shift 5 carried ~~~$12–48/mo~~ from the teardown — one column's promo floor mashed with the other's list ceiling, caught at the competitor-salesperson gate r1; shift 12 corrected it to ~$20–48 list / ~$12–29 promo from the teardown's own table; shift 13 wrote OBSERVED here on Phin's capture — a grade the legend reserves for *"real usage, n stated"*, which a pricing page is not. This cell and the next state one grade.)* | **SOURCED — RULED by the chief 2026-09-07T22:42Z (law 7 revaluation). The row read UNSETTLED from shift 23 because its two cells stated two grades, and neither was right:** the value cell said OBSERVED, and the legend defines OBSERVED as *"real usage, n stated"* — a competitor's pricing page is not usage and carries no n; this cell said DIRECTIONAL and named the teardown, which is not the row's source. The source is *"a document the org read, named"*: **Phin's primary capture of 2026-09-03** (`gtm/market/competitors/snapshots/2026-09-03-holo-primary.md` §1 — *"TWO PLANS ONLY"*; List price **$20** / **$48**; Promo price **$12/mo** / **$29/mo**; Creatives per month **120** / **350**). **Two limitations travel on the face of this row with the grade. (1) The capture is a human paste with no hash and no fetch record** — its own header: *"pasted verbatim into the chief's channel"*; `grep -in 'sha\|hash\|http 200\|bytes\|curl'` over the file, run 2026-09-07T22:48Z, returns two lines (141, 219), both prose hits on *"shares"* and *"sharper"*, not a fetch record. **(2) The ruling carried *"`tryholo.ai` is egress-blocked to every agent here, so the row cannot be refreshed by this org."* Tried this shift before writing it down, and it did not reproduce:** `curl -sS -m 30 https://tryholo.ai/pricing` at 2026-09-07T22:49:06Z returned **HTTP 200, 198,978 B, SHA-256 `8bc11e1f…`**, server-rendered, and its extracted text carries **$20 / $48 (struck, beside the promo), $12 / $29, *"120 creatives every month"* / *"350 creatives every month"*, two plan names, *"4,268 brands"*** — every A15 figure in the primary, four days on. Receipt: `gtm/market/competitors/snapshots/2026-09-07-holo-pricing-fetch.md` (extracted text retained, both hashed). **So as of this date the refresh path is a machine fetch from this box, and the capture date is still the thing that decays** — 2026-09-03 for the source, 2026-09-07 for the corroborating fetch. *Blind spot of the fetch, same breath: raw HTML through the session proxy, no JavaScript run, not logged in, one region, one moment — and four shifts recorded a block from boxes behind the same kind of proxy, so which side changed is not known and the next fetch may be blocked again.* **Whether the hashed fetch replaces the paste as the row's named document is a revaluation and is not the boss's.** **Corroborating citation, not the source:** the Holo teardown (`clinkworthy/docs/competitor-holo.md` pricing table, third-party-sourced, DIRECTIONAL — never load-bearing alone), which keeps its own flag *SPOT-CHECK before any external use* under the STATED-LIMITATION clause; a floor, not a description. Graded against the primary (shift 23; the two verdict tables re-derived at the round-2 gate): **its price band is CONFIRMED; its plan count (three), creative allowances (200 / 650), Trustpilot count, refund terms, BLUF and *"generates on demand, then silence"* are FALSE.** No ratio — none has a denominator rule. Receipt for that grading: `snapshots/2026-09-07-shift25-holo-teardown-grade.md` (the re-cut of the pulled §4; DRAFT, ungated). Nothing cites the pulled §4, this row included. The Trustpilot and refund corrections live at A35 and A34. **UNGATED — wording by the boss shift 25; the grade is the chief's ruling.** |
